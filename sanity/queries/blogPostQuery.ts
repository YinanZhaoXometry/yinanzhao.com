...
export const fetchBlogPost = (slug: string) =>
  client.fetch<PostDetail | undefined, { slug: string }>(BLOG_POST_QUERY, {
    slug,
  })

export const ALL_LATEST_BLOG_POST_SLUGS_QUERY = groq`
...
export const getAllLatestBlogPostSlugs = () => {
  return client.fetch<string[]>(ALL_LATEST_BLOG_POST_SLUGS_QUERY)
}
...
export const makeLatestBlogPostsQuery = ({
  limit = 5,
  forDisplay = true,
}: GetBlogPostsOptions) =>
  groq`
...
export const fetchLatestBlogPosts = (options: GetBlogPostsOptions) =>
  client.fetch<Post[] | null>(makeLatestBlogPostsQuery(options))
